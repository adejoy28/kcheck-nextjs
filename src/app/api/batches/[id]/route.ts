...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        await sql`
            UPDATE batches SET name = ${name}, start_date = ${start_date},
            end_date = ${end_date}, is_active = ${is_active ?? true}, updated_at = NOW()
            WHERE id = ${params.id}
        `

        await sql`DELETE FROM batch_members WHERE batch_id = ${params.id}` 
        await sql`DELETE FROM batch_teams WHERE batch_id = ${params.id}` 

        if (user_ids?.length > 0) {
            for (const userId of user_ids) {
                await sql`INSERT INTO batch_members (batch_id, user_id) VALUES (${params.id}, ${userId}) ON CONFLICT DO NOTHING` 
            }
        }
        if (team_ids?.length > 0) {
            for (const teamId of team_ids) {
                await sql`INSERT INTO batch_teams (batch_id, team_id) VALUES (${params.id}, ${teamId}) ON CONFLICT DO NOTHING` 
            }
        }
